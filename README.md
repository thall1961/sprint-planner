...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create sprint-planner-3af0
  fly apps create sprint-planner-3af0-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app sprint-planner-3af0
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app sprint-planner-3af0-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'sprint-planner-3af0-staging' does not match app name in config file 'sprint-planner-3af0'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name sprint-planner-3af0-db
  fly postgres attach --app sprint-planner-3af0 sprint-planner-3af0-db

  fly postgres create --name sprint-planner-3af0-staging-db
  fly postgres attach --app sprint-planner-3af0-staging sprint-planner-3af0-staging-db
  ```
